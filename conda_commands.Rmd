...
or for a more specific search type the package names follow by the _regex_ `|`:
...
To _activate_ an environment, you simply use `conda activate env-name`. To _deactivate_ an environment, you use `conda deactivate`, which returns you to the root/base environment. It's also possible to switch between environments. Example:
